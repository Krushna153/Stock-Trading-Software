<?php 
	...
	//
	//find last orderid
	$lastordsql="select order_id FROM sellorders ORDER BY order_id DESC LIMIT 1";
	$lastordid=0;
	...
	$sql="insert into sellorders (order_id,client_id,order_type,position_type,ticker_name,ticker_size,price,status) values('".$lastordid."','".$client_id."',$ordertype,$positiontype,'".$tickername."',$stocksize,$stockprice,\"Pending\")";
	...
	$sql2="select sellorders.status from sellorders where order_id= '".$lastordid."'";
	...
	if($state==="Failed"){
		$msg="Sell Order failed (Invalid Quantity)!";
	}else{
